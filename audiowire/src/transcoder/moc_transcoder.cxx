/****************************************************************************
** Meta object code from reading C++ file 'transcoder.h'
**
** Created: Tue Apr 23 15:28:14 2013
**      by: The Qt Meta Object Compiler version 63 (Qt 4.8.3)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "transcoder.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'transcoder.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.3. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_Transcoder[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       3,       // signalCount

 // signals: signature, parameters, type, tag, flags
      29,   12,   11,   11, 0x05,
      63,   55,   11,   11, 0x05,
      80,   11,   11,   11, 0x05,

 // slots: signature, parameters, type, tag, flags
      98,   11,   11,   11, 0x0a,
     106,   11,   11,   11, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_Transcoder[] = {
    "Transcoder\0\0filename,success\0"
    "JobComplete(QString,bool)\0message\0"
    "LogLine(QString)\0AllJobsComplete()\0"
    "Start()\0Cancel()\0"
};

void Transcoder::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        Transcoder *_t = static_cast<Transcoder *>(_o);
        switch (_id) {
        case 0: _t->JobComplete((*reinterpret_cast< const QString(*)>(_a[1])),(*reinterpret_cast< bool(*)>(_a[2]))); break;
        case 1: _t->LogLine((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 2: _t->AllJobsComplete(); break;
        case 3: _t->Start(); break;
        case 4: _t->Cancel(); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData Transcoder::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject Transcoder::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_Transcoder,
      qt_meta_data_Transcoder, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &Transcoder::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *Transcoder::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *Transcoder::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_Transcoder))
        return static_cast<void*>(const_cast< Transcoder*>(this));
    return QObject::qt_metacast(_clname);
}

int Transcoder::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    }
    return _id;
}

// SIGNAL 0
void Transcoder::JobComplete(const QString & _t1, bool _t2)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)), const_cast<void*>(reinterpret_cast<const void*>(&_t2)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void Transcoder::LogLine(const QString & _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}

// SIGNAL 2
void Transcoder::AllJobsComplete()
{
    QMetaObject::activate(this, &staticMetaObject, 2, 0);
}
QT_END_MOC_NAMESPACE
