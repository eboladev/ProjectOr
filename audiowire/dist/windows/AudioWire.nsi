!define PRODUCT_NAME "AudioWire"
!define PRODUCT_PUBLISHER "AudioWire"
...
!define PRODUCT_WEB_SITE "http://www.AudioWire-player.org/"
!define PRODUCT_UNINST_KEY "Software\Microsoft\Windows\CurrentVersion\Uninstall\${PRODUCT_NAME}"
!define PRODUCT_UNINST_ROOT_KEY "HKLM"
!define PRODUCT_INSTALL_DIR "$PROGRAMFILES\AudioWire"

!define WMDM_DIST_URL "http://AudioWire-player.googlecode.com/files/wmdmdist.exe"
!define WMF_DIST_URL  "http://AudioWire-player.googlecode.com/files/wmfdist.exe"

; Set Application Capabilities info
!define CAPABILITIES_NAME "AudioWire"
!define CAPABILITIES_LOCAL_NAME "AudioWire"
!define CAPABILITIES_PROGID "AudioWire music player"
!define CAPABILITIES_PATH "Software\Clients\Media\AudioWire"
!define CAPABILITIES_DESCRIPTION "AudioWire music player"
!define CAPABILITIES_ICON "$INSTDIR\AudioWire.ico"
...
!define MUI_ICON "AudioWire.ico"

!define MUI_COMPONENTSPAGE_SMALLDESC
!define MUI_FINISHPAGE_RUN $INSTDIR/AudioWire.exe
...
Section "AudioWire" AudioWire
  ...
  File "avutil-51.dll"
  File "AudioWire.exe"
  File "AudioWire-tagreader.exe"
  File "AudioWire-spotifyblob.exe"
  File "AudioWire.ico"
  File "glew32.dll"
  File "intl.dll"
  File "libcdio-14.dll"
  ...
  HasDefaultPrograms:
    ; Register AudioWire with Default Programs
    Var /GLOBAL AppIcon
    Var /GLOBAL AppExe
    StrCpy $AppExe "$INSTDIR\AudioWire.exe"
    StrCpy $AppIcon "$INSTDIR\AudioWire.ico"
    ...
    Goto done
  NoDefaultPrograms:
    ${registerExtension} "$INSTDIR\AudioWire.exe" ".mp3" "MP3 Audio File"
    ${registerExtension} "$INSTDIR\AudioWire.exe" ".flac" "FLAC Audio File"
    ${registerExtension} "$INSTDIR\AudioWire.exe" ".ogg" "OGG Audio File"
    ${registerExtension} "$INSTDIR\AudioWire.exe" ".spx" "OGG Speex Audio File"
    ${registerExtension} "$INSTDIR\AudioWire.exe" ".m4a" "MP4 Audio File"
    ${registerExtension} "$INSTDIR\AudioWire.exe" ".aac" "AAC Audio File"
    ${registerExtension} "$INSTDIR\AudioWire.exe" ".wma" "WMA Audio File"
    ${registerExtension} "$INSTDIR\AudioWire.exe" ".wav" "WAV Audio File"

    ${registerExtension} "$INSTDIR\AudioWire.exe" ".pls" "PLS Audio File"
    ${registerExtension} "$INSTDIR\AudioWire.exe" ".m3u" "M3U Audio File"
    ${registerExtension} "$INSTDIR\AudioWire.exe" ".xspf" "XSPF Audio File"
    ${registerExtension} "$INSTDIR\AudioWire.exe" ".asx" "Windows Media Audio/Video playlist"
  done:
SectionEnd
...
Section "Start menu items" startmenu
  ...
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\AudioWire.exe"
...
Section "Uninstaller"
  ...
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\AudioWire.ico"
...
Section "Uninstall"
  ; Kill AudioWire.exe if it's running
  ; This calling convention is retarded...
  StrCpy $0 "AudioWire.exe"
  KillProc::FindProcesses
  StrCmp $1 "-1" wooops

  StrCmp $0 "0" completed
  
  DetailPrint "Killing running AudioWire.exe..."
  ...
  wooops:
  DetailPrint "-> Error: Something went wrong while killing running AudioWire.exe"
  Abort
  ...
  Delete "$INSTDIR\avformat-52.dll"
  Delete "$INSTDIR\avutil-50.dll"
  Delete "$INSTDIR\AudioWire.ico"
  Delete "$INSTDIR\AudioWire.exe"
  Delete "$INSTDIR\AudioWire-tagreader.exe"
  Delete "$INSTDIR\AudioWire-spotifyblob.exe"
